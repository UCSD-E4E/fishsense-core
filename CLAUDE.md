...
- Wheels published: cp312 / cp313 / cp314 manylinux_2_34 x86_64, in two flavors — the default CPU build (`fishsense_core-<X.Y.Z>-…`) and a CUDA build (`fishsense_core-<X.Y.Z>+cu12-…`, built with `--features cuda`). The `+cu12` wheels are not self-contained: ORT's CUDA EP resolves `libcudart`/`libcudnn`/etc. at runtime from the `nvidia-*-cu12` pip packages that `_preload_nvidia_libs()` loads, so install those alongside it. No Windows, macOS, or aarch64 wheels. No PyPI publish — wheels live only as GitHub Release assets (and PyPI would reject the `+cu12` local version anyway).
